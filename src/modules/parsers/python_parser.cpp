...
{
  public:
    explicit PythonParser() = default;

    ~PythonParser() override = default;

    // parses the file content by the rules of the Python parser
    [[nodiscard]] bool parse(std::string buffer) override
    {
        ...
        while (line_start < m_buffer.size())
        {
            ...
            while (i < line.size())
            {
                if (block_comment)
                {
                    size_t current_line_end = line.find(BLOCK_COMMENT, i);
